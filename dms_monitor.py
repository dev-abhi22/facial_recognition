import cv2
import math
import json
import time
import os
import uuid
from collections import deque

import keyboard
from cvzone.FaceMeshModule import FaceMeshDetector
import pathlib

# ============================================================
# CONFIG
# ============================================================
STATE_FILE = pathlib.Path(__file__).parent / "dms_state.json"

MAR_THRESH = 0.30
ROLL_THRESH = 30
ALARM_FRAMES_SLEEP = 60
ALARM_FRAMES_WOBBLE = 45
ALARM_FRAMES_YAWN = 20

WEIGHT_STEERING = 15
WEIGHT_SLEEP = 10
WEIGHT_WOBBLE = 5
WEIGHT_YAWN = 2

RISK_THRESHOLD = 500
RISK_RESET_FRAMES = 300

TURN_SPEED = 8
MAX_ANGLE = 180
BUFFER_SIZE = 45


# ============================================================
# STATE HELPERS
# ============================================================
def default_state():
    return {
        "timestamp": time.time(),
        "auto_engaged": False,
        "manual_override": False,
        "risk_score": 0,
        "sleep_frames": 0,
        "wobble_frames": 0,
        "yawn_count": 0,
        "volatility": 0,
        "ear": 0.0,
        "mar": 0.0,
        "pitch": 0.0,
        "yaw_face": 0.0,
        "roll": 0.0,
        "warnings": [],
        "calibrated": False
    }


def write_state(data, retries=10, delay=0.01):
    """
    Windows-safe shared-state writer.
    Uses a unique temp file, retries atomic replace, then falls back to direct write.
    """
    tmp = f"{STATE_FILE}.{uuid.uuid4().hex}.tmp"

    with open(tmp, "w", encoding="utf-8") as f:
        json.dump(data, f, indent=2)
        f.flush()
        os.fsync(f.fileno())

    last_err = None
    for _ in range(retries):
        try:
            os.replace(tmp, STATE_FILE)
            return
        except PermissionError as e:
            last_err = e
            time.sleep(delay)
        except OSError as e:
            last_err = e
            time.sleep(delay)

    try:
        with open(STATE_FILE, "w", encoding="utf-8") as f:
            json.dump(data, f, indent=2)
            f.flush()
            os.fsync(f.fileno())
    except Exception as e:
        print(f"[DMS] Failed to write state: {e}")
    finally:
        if os.path.exists(tmp):
            try:
                os.remove(tmp)
            except OSError:
                pass

    if last_err is not None:
        print(f"[DMS] Warning: state-file replace had sharing issues: {last_err}")


# ============================================================
# INIT
# ============================================================
cap = cv2.VideoCapture(0)
detector = FaceMeshDetector(maxFaces=1)

if not cap.isOpened():
    raise RuntimeError("Could not open webcam")

angle_changes = deque(maxlen=BUFFER_SIZE)
current_angle = 0
prev_angle = 0

sleep_frames = 0
yawn_frames = 0
yawn_numbers = 0
wobble_frames = 0
risk_score = 0
risk_timer = 0
auto_engaged = False

write_state(default_state())


# ============================================================
# CALIBRATION
# ============================================================
print("Calibrating driver baseline... Look straight at the camera.")

ear_vals = []
pitch_vals = []
yaw_vals = []

while True:
    success, img = cap.read()
    if not success:
        continue

    img, faces = detector.findFaceMesh(img, draw=True)

    if faces:
        face = faces[0]

        l_vert, _ = detector.findDistance(face[159], face[145])
        l_horz, _ = detector.findDistance(face[130], face[133])
        r_vert, _ = detector.findDistance(face[386], face[374])
        r_horz, _ = detector.findDistance(face[362], face[263])

        ear = ((l_vert / l_horz if l_horz else 0.0) +
               (r_vert / r_horz if r_horz else 0.0)) / 2.0
        ear_vals.append(ear)

        nose, chin, forehead = face[1], face[152], face[10]
        nc, _ = detector.findDistance(nose, chin)
        nf, _ = detector.findDistance(nose, forehead)
        pitch = nc / nf if nf else 0.0
        pitch_vals.append(pitch)

        nl, _ = detector.findDistance(nose, face[234])
        nr, _ = detector.findDistance(nose, face[454])
        yaw_face = nl / nr if nr else 1.0
        yaw_vals.append(yaw_face)

        if len(pitch_vals) >= 30:
            break

    pct = int(100 * len(pitch_vals) / 30)
    bar = "█" * (pct // 5) + "░" * (20 - pct // 5)
    cv2.putText(img, f"CALIBRATING [{bar}] {pct}%",
                (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 200, 255), 2)
    cv2.imshow("DMS Monitor", img)

    if cv2.waitKey(1) & 0xFF == ord("q"):
        cap.release()
        cv2.destroyAllWindows()
        raise SystemExit

EAR_THRESH = sum(ear_vals) / max(len(ear_vals), 1)
PITCH_THRESH = sum(pitch_vals) / max(len(pitch_vals), 1)
YAW_BASE = sum(yaw_vals) / max(len(yaw_vals), 1)
YAW_MIN = YAW_BASE * 0.2
YAW_MAX = YAW_BASE * 1.8

print(f"Calibration complete: EAR={EAR_THRESH:.3f}, PITCH={PITCH_THRESH:.3f}, YAW={YAW_BASE:.3f}")


# ============================================================
# MAIN LOOP
# ============================================================
avg_ear = 0.0
mar = 0.0
pitch = 0.0
yaw_face = 0.0
roll = 0.0

while True:
    success, img = cap.read()
    if not success:
        continue

    h, w, _ = img.shape
    wheel_center = (w - 80, h - 80)
    wheel_radius = 60

    # Fake steering volatility using keyboard taps
    if keyboard.is_pressed("left"):
        current_angle -= TURN_SPEED
    if keyboard.is_pressed("right"):
        current_angle += TURN_SPEED
    current_angle = max(-MAX_ANGLE, min(MAX_ANGLE, current_angle))

    delta = abs(current_angle - prev_angle)
    angle_changes.append(delta)
    prev_angle = current_angle
    volatility = sum(angle_changes)

    img, faces = detector.findFaceMesh(img, draw=True)
    current_frame_risk = 0
    warnings = []

    if faces:
        face = faces[0]

        l_vert, _ = detector.findDistance(face[159], face[145])
        l_horz, _ = detector.findDistance(face[130], face[133])
        r_vert, _ = detector.findDistance(face[386], face[374])
        r_horz, _ = detector.findDistance(face[362], face[263])

        avg_ear = ((l_vert / l_horz if l_horz else 0.0) +
                   (r_vert / r_horz if r_horz else 0.0)) / 2.0

        m_vert, _ = detector.findDistance(face[11], face[16])
        m_horz, _ = detector.findDistance(face[78], face[308])
        mar = m_vert / m_horz if m_horz else 0.0

        nose, chin, forehead = face[1], face[152], face[10]
        nc, _ = detector.findDistance(nose, chin)
        nf, _ = detector.findDistance(nose, forehead)
        pitch = nc / nf if nf else 0.0

        nl, _ = detector.findDistance(nose, face[234])
        nr, _ = detector.findDistance(nose, face[454])
        yaw_face = nl / nr if nr else 1.0

        leo, reo = face[33], face[263]
        roll = math.degrees(math.atan2(reo[1] - leo[1], reo[0] - leo[0]))

        if avg_ear < 0.8 * EAR_THRESH:
            sleep_frames += 1
        else:
            sleep_frames = max(0, sleep_frames - 1)

        if mar > MAR_THRESH:
            yawn_frames += 1
        else:
            yawn_frames = 0

        if yawn_frames > ALARM_FRAMES_YAWN:
            yawn_numbers += 1
            yawn_frames = 0

        if ((pitch < 0.67 * PITCH_THRESH or pitch > 3 * PITCH_THRESH) or
            (yaw_face < YAW_MIN or yaw_face > YAW_MAX) or
            abs(roll) > ROLL_THRESH):
            wobble_frames += 1
        else:
            wobble_frames = max(0, wobble_frames - 2)

    if sleep_frames > ALARM_FRAMES_SLEEP:
        warnings.append("DANGER: DRIVER ASLEEP")
        current_frame_risk += WEIGHT_SLEEP

    if wobble_frames > ALARM_FRAMES_WOBBLE:
        warnings.append("WARNING: HEAD DISTRACTED")
        current_frame_risk += WEIGHT_WOBBLE

    if yawn_numbers > 5:
        warnings.append("WARNING: EXCESSIVE YAWNING")
        current_frame_risk += WEIGHT_YAWN

    if volatility > 180:
        warnings.append("DANGER: ERRATIC STEERING")
        current_frame_risk += WEIGHT_STEERING

    if not auto_engaged:
        risk_score += current_frame_risk

    risk_timer += 1
    if risk_timer > RISK_RESET_FRAMES:
        if not auto_engaged:
            risk_score = 0
        yawn_numbers = 0
        risk_timer = 0

    if risk_score > RISK_THRESHOLD:
        auto_engaged = True

    manual_override = False
    if keyboard.is_pressed("x"):
        auto_engaged = False
        risk_score = 0
        risk_timer = 0
        yawn_numbers = 0
        sleep_frames = 0
        wobble_frames = 0
        manual_override = True
        print("[DMS] Manual override requested")

    for i, text in enumerate(warnings):
        cv2.putText(img, text, (20, 160 + i * 35),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 0, 255), 2)

    mode_text = "AUTO TAKEOVER REQUESTED" if auto_engaged else "MANUAL MONITORING"
    mode_color = (0, 0, 255) if auto_engaged else (0, 255, 0)
    cv2.putText(img, mode_text, (20, 50),
                cv2.FONT_HERSHEY_SIMPLEX, 1.0, mode_color, 3)

    cv2.putText(img,
                f"EAR:{avg_ear:.2f} MAR:{mar:.2f} P:{pitch:.2f} Y:{yaw_face:.2f} R:{int(roll)}",
                (20, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
    cv2.putText(img, f"Risk:{risk_score}/{RISK_THRESHOLD}",
                (20, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.7, mode_color, 2)
    cv2.putText(img, f"Vol:{volatility}",
                (w - 180, h - 130), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
    cv2.putText(img, "Press X for manual reset",
                (20, h - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 2)

    cv2.circle(img, wheel_center, wheel_radius, (255, 255, 255), 4)
    theta_rad = math.radians(current_angle - 90)
    end_x = int(wheel_center[0] + wheel_radius * math.cos(theta_rad))
    end_y = int(wheel_center[1] + wheel_radius * math.sin(theta_rad))
    cv2.line(img, wheel_center, (end_x, end_y), (255, 255, 255), 4)
    cv2.circle(img, wheel_center, 10, (255, 255, 255), cv2.FILLED)

    state = {
        "timestamp": time.time(),
        "auto_engaged": auto_engaged,
        "manual_override": manual_override,
        "risk_score": risk_score,
        "sleep_frames": sleep_frames,
        "wobble_frames": wobble_frames,
        "yawn_count": yawn_numbers,
        "volatility": volatility,
        "ear": avg_ear,
        "mar": mar,
        "pitch": pitch,
        "yaw_face": yaw_face,
        "roll": roll,
        "warnings": warnings,
        "calibrated": True
    }
    write_state(state)

    cv2.imshow("DMS Monitor", img)
    key = cv2.waitKey(1) & 0xFF
    if key == ord("q"):
        break

print("Shutting down DMS...")
write_state(default_state())
cap.release()
cv2.destroyAllWindows()